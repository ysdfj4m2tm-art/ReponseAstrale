...
export function HeaderNavigation({ account }: { account: HeaderAccount }) {
  ...
  const authControl = account ? (
    <Link href="/espace" className="account-summary" aria-label={`Mon espace, ${formatSunCount(account.suns)} restant${account.suns > 1 ? "s" : ""}`}>
      ...
      <span className="account-summary__balance"><SunTokenIcon />{formatSunCount(account.suns)}</span>
  ...
  return <><header className="site-header">
    <div className="shell header-inner">
      ...
      <div className="header-account-mobile">{account ? <Link href="/espace" className="mobile-account-trigger" aria-label={`Mon espace, ${formatSunCount(account.suns)}`} title={account.email}><CircleUserRound/><span><SunTokenIcon/>{formatSunCount(account.suns)}</span></Link> : <Link href="/connexion" className="header-login">Se connecter</Link>}</div>
